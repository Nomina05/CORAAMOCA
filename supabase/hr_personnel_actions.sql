...
declare v_user public.app_users;v_id uuid:=nullif(p_data->>'id','')::uuid;v_employee public.hr_employees;v_existing public.hr_personnel_actions;v_type text:=p_data->>'action_type';v_effective date:=(p_data->>'effective_date')::date;v_exit_cause text;begin v_user:=public.hr_authenticated_user(p_token);
 ...
 if v_id is not null then
  ...
  update public.hr_personnel_actions set employee_id=v_employee.id,effective_date=v_effective,action_type=v_type,other_action=coalesce(p_data->>'other_action',''),proposed_state=coalesce(p_data->'proposed_state','{}'::jsonb),observations=coalesce(p_data->>'observations',''),recommendation=coalesce(nullif(p_data->>'recommendation',''),v_existing.recommendation),immediate_supervisor=coalesce(nullif(p_data->>'immediate_supervisor',''),v_existing.immediate_supervisor),department_manager=coalesce(nullif(p_data->>'department_manager',''),v_existing.department_manager),payroll_officer=coalesce(nullif(p_data->>'payroll_officer',''),v_existing.payroll_officer),general_director=coalesce(nullif(p_data->>'general_director',''),v_existing.general_director),approval_dates=jsonb_build_object('recommendation_date',coalesce(nullif(p_data#>>'{approval_dates,recommendation_date}',''),v_existing.approval_dates->>'recommendation_date',''),'department_date',coalesce(nullif(p_data#>>'{approval_dates,department_date}',''),v_existing.approval_dates->>'department_date',''),'payroll_date',coalesce(nullif(p_data#>>'{approval_dates,payroll_date}',''),v_existing.approval_dates->>'payroll_date',''),'director_date',coalesce(nullif(p_data#>>'{approval_dates,director_date}',''),v_existing.approval_dates->>'director_date','')),exit_date=case when v_exit_cause<>'' then v_effective else null end,exit_cause=v_exit_cause,updated_at=now() where id=v_id;
 else
