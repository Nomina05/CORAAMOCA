...
returns jsonb language plpgsql security definer set search_path=public,extensions as $$
declare v_user public.app_users;v_id uuid;v_employee public.hr_employees;v_type text:=p_data->>'action_type';begin v_user:=public.hr_authenticated_user(p_token);
 ...
 if v_type in ('DESPIDO','ABANDONO_TRABAJO','RENUNCIA','RESCISION_CONTRATO') and (nullif(p_data->>'exit_date','') is null or nullif(trim(coalesce(p_data->>'exit_cause','')),'') is null) then return jsonb_build_object('success',false,'error','La fecha y la causa de salida son obligatorias para terminar la relación laboral.');end if;
 ...
 coalesce(p_data->'proposed_state','{}'::jsonb),coalesce(p_data->>'observations',''),coalesce(p_data->>'recommendation',''),coalesce(p_data->>'immediate_supervisor',''),coalesce(p_data->>'department_manager',''),coalesce(p_data->>'payroll_officer',''),coalesce(p_data->>'general_director',''),nullif(p_data->>'exit_date','')::date,coalesce(p_data->>'exit_cause',''),v_user.id) returning id into v_id;
