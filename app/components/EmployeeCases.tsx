...
export default function EmployeeCases({type,canCreate=false,canApprove=false}:{type:CaseType;canCreate?:boolean;canApprove?:boolean}){
 ...
 const load=useCallback(async()=>{setBusy(true);try{const response=await fetch(`/api/hr/employee-cases?type=${type}&year=${year}`,{cache:"no-store"}),data=await response.json();setItems(response.ok&&Array.isArray(data.items)?data.items:[]);if(!response.ok)setMessage(data.error||"No fue posible cargar los registros.")}finally{setBusy(false)}},[type,year]);
 useEffect(()=>{fetch("/api/hr/employees",{cache:"no-store"}).then(r=>r.json()).then(d=>setEmployees(Array.isArray(d.employees)?d.employees.filter((e:Employee)=>e.employment_status!=="Desvinculado"):[]));load()},[load]);
