...
type Item={id:string;case_number:number;employee_code:string;full_name:string;position_name:string;direction_name:string;case_type:CaseType;request_date:string;category:string;start_date:string;end_date:string;day_count:number;paid:boolean;severity:string;reason:string;observations:string;status:string;created_by_name:string;approved_by_name?:string};
const configs={
...
export default function EmployeeCases({type,canCreate=false,canApprove=false}:{type:CaseType;canCreate?:boolean;canApprove?:boolean}){
 ...
 return <section className="employee-registry-panel">
  ...
  <div className="employee-registry-head"><div><span className="eyebrow">{current.eyebrow}</span><h2>{current.title}</h2><p>{current.description}</p></div><div><label>Período histórico<select value={year} onChange={e=>setYear(e.target.value)}><option value="ALL">Todos los años</option>{Array.from({length:10},(_,i)=>2026+i).map(y=><option key={y} value={String(y)}>{y}</option>)}</select></label></div></div>
  ...
  <div className="technical-card"><div className="users-card-head"><div><h2>Histórico de {current.title.toLowerCase()}</h2><p>Consulta por año o todos los años, vinculada al expediente maestro y a la auditoría institucional.</p></div><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Buscar empleado, motivo o área"/></div>
   {busy&&!items.length?<div className="users-empty">Cargando registros…</div>:!filtered.length?<div className="technical-empty"><strong>No existen registros para este período</strong><span>Utilice el formulario superior para iniciar el módulo.</span></div>:<div className="users-table-wrap"><table className="employee-registry-table"><thead><tr><th>NO. / COLABORADOR</th><th>TIPO</th><th>SOLICITUD / PERÍODO</th><th>MOTIVO</th><th>ESTATUS</th><th>RESPONSABLE</th><th>ACCIONES</th></tr></thead><tbody>{filtered.map(item=><tr key={item.id}>
    <td><strong>#{item.case_number} · {item.full_name}</strong><small>{item.employee_code} · {item.position_name}</small></td><td><strong>{item.category}</strong><small>{item.severity||(type==="PERMISO"?(item.paid?"Con salario":"Sin salario"):"")}</small></td><td><strong>{type==="AMONESTACION"?"Fecha del hecho":"Solicitado"}: {date(item.request_date)}</strong><small>{type!=="AMONESTACION"?`Desde ${date(item.start_date)} hasta ${date(item.end_date)} · ${item.day_count} días`:""}</small></td><td>{item.reason}<small>{item.observations}</small></td><td><span className={`user-status ${item.status==="APROBADO"||item.status==="NOTIFICADA"?"is-active":"is-inactive"}`}>{item.status}</span></td><td>{item.created_by_name}<small>{item.approved_by_name?`Procesado por ${item.approved_by_name}`:"Pendiente de decisión"}</small></td><td>{canApprove&&(item.status==="SOLICITADO"||item.status==="REGISTRADA")?<div><button className="row-action" onClick={()=>decide(item,"APROBAR")}>{type==="AMONESTACION"?"Notificar":"Aprobar"}</button> <button className="row-action" onClick={()=>decide(item,"RECHAZAR")}>Rechazar</button></div>:"—"}</td>
   </tr>)}</tbody></table></div>}
  </div>
