...
const organizationalStructure:OrganizationUnit={name:"Dirección General",children:[
 ...
 {name:"Departamento de Control y Análisis de Operaciones"},{name:"Departamento Jurídico"},{name:"Oficina de Acceso a la Información"}
]};
...
export default function Home() {
  ...
  useEffect(()=>{if(!currentUser||currentUser.role==="Administrador")return;const views:Array<[string,keyof Permissions]>=[["Resumen","ver_resumen"],["Proyectos","ver_proyectos"],["Proyectos Técnicos","ver_proyectos_tecnicos"],["Cubicaciones","ver_cubicaciones"],["Calendario","ver_calendario"],["Reportes","ver_reportes"]];const allowed=views.filter(([,key])=>Boolean(currentUser.permissions?.[key])).map(([name])=>name);if(currentUser.area==="Gestión Humana")allowed.push("Recursos Humanos","Estructura Organizacional");if((currentUser.role==="Director")&&!allowed.includes("Catálogos"))allowed.push("Catálogos");if(!allowed.includes(section))setSection(allowed[0]||"Acceso restringido");},[currentUser,section]);
