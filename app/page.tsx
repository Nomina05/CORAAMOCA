...
function PublicInvestmentReport({rows}:{rows:PublicInvestmentRow[]}){
  ...
  const totalAwarded=sum(rows,"awarded_amount"),totalMeasured=sum(rows,"total_measured"),totalPaid=sum(rows,"total_paid");
  return <div className="public-investment-report">
    ...
    {groups.map(([key,items])=><section className="public-location" key={key}>
      <div className="public-location-head"><strong>{items[0].location_type}: {items[0].location_name}</strong><span>Población: {sum(items,"population").toLocaleString("es-DO")}</span><span>Inversión: {money(sum(items,"awarded_amount"))}</span><span>{sum(items,"linear_meters").toLocaleString("es-DO")} ML</span></div>
      <div className="users-table-wrap"><table className="report-table public-investment-table"><thead><tr><th>CÓDIGO SNIP</th><th>TIPO DE OBRA</th><th>SECTOR</th><th>MONTO ADJUDICADO</th><th>AVANCE 20 %</th><th>ESTATUS DE CUBICACIÓN</th><th>CUBICADO</th><th>TOTAL PAGADO</th><th>ESTATUS</th><th>% DE LA OBRA</th></tr></thead><tbody>
        {items.map(item=><tr key={item.id}><td>{item.snip_code||"Sin SNIP"}</td><td><strong>{item.work_type||item.work_name}</strong></td><td>{item.sector||"Sin sector"}</td><td>{money(Number(item.awarded_amount))}</td><td>{money(Number(item.advance_20_amount))}</td><td>{item.measurement_status||"Pendiente"}</td><td>{money(Number(item.total_measured))}</td><td>{money(Number(item.total_paid))}</td><td>{item.work_status||"Sin estatus"}</td><td>{Number(item.work_percentage).toFixed(2)} %</td></tr>)}
    ...
    <section className="public-general-summary"><h3>RESUMEN GENERAL CON CÓDIGOS SNIP / OBRAS</h3><div><span><b>Inversión adjudicada</b>{money(totalAwarded)}</span><span><b>Total cubicado</b>{money(totalMeasured)}</span><span><b>Total pagado en obras</b>{money(totalPaid)}</span><span><b>Balance pendiente</b>{money(totalAwarded-totalPaid)}</span></div></section>
  </div>;
}
