...
function PublicInvestmentReport({rows}:{rows:PublicInvestmentRow[]}){
  const groups=Array.from(rows.reduce((map,row)=>{const key=`${row.location_type}:${row.location_name}`;const current=map.get(key)||[];current.push(row);map.set(key,current);return map;},new Map<string,PublicInvestmentRow[]>()).entries());
  ...
  return <div className="public-investment-report">
    ...
    {groups.map(([key,items])=><section className="public-location" key={key}>
      ...
      <div className="users-table-wrap"><table className="report-table public-investment-table"><thead><tr><th>SNIP</th><th>OBRAS O ACTIVOS</th><th>SECTOR</th><th>MONTO ADJUDICADO</th><th>AVANCE 20 %</th><th>ESTATUS DE CUBICACIÓN</th><th>CUBICADO</th><th>TOTAL PAGADO</th><th>ESTATUS DE OBRA</th><th>% DE OBRA EJECUTADA / AVANCE FÍSICO</th></tr></thead><tbody>
        ...
        <tr className="public-subtotal"><td colSpan={3}>TOTAL {items[0].location_name.toUpperCase()}</td><td>{money(sum(items,"awarded_amount"))}</td><td>{money(sum(items,"advance_20_amount"))}</td><td/><td>{money(sum(items,"total_measured"))}</td><td>{money(sum(items,"total_paid"))}</td><td colSpan={2}/></tr>
      </tbody></table></div>
    </section>)}
