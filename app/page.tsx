...
export default function Home() {
  ...
  useEffect(()=>{
    ...
    let active=true;
    const loadNotifications=()=>fetch("/api/notifications",{cache:"no-store"}).then(response=>response.ok?response.json():null).then(data=>{if(active)setNotice((data?.items||[]).filter((item:{read_at?:string|null})=>!item.read_at).length)});
    loadNotifications();
    const interval=window.setInterval(loadNotifications,30000);
    return()=>{active=false;window.clearInterval(interval)};
  },[currentUser?.id]);

  useEffect(()=>{if(!currentUser||currentUser.role==="Administrador")return;const views:Array<[string,keyof Permissions]>=[["Resumen","ver_resumen"],["Proyectos","ver_proyectos"],["Proyectos Técnicos","ver_proyectos_tecnicos"],["Cubicaciones","ver_cubicaciones"],["Gestión Presupuestaria","ver_gestion_presupuestaria"],["Calendario","ver_calendario"],["Reportes","ver_reportes"],["Recursos Humanos","ver_recursos_humanos"],["Estructura Organizacional","ver_estructura_organizacional"],["Catálogos","ver_catalogos"]];const allowed=views.filter(([,key])=>Boolean(currentUser.permissions?.[key])).map(([name])=>name);if(!allowed.includes(section))setSection(allowed[0]||"Acceso restringido");},[currentUser,section]);
    ...
    <div className="app-shell">
      ...
      <main>
        ...
        <div className="content">
          ...
          <div className={section === "Resumen" || section === "Usuarios" || section === "Catálogos" || section === "Proyectos Técnicos" || section === "Cubicaciones" || section === "Gestión Presupuestaria" || section === "Recursos Humanos" || section === "Estructura Organizacional" || section === "Acceso restringido" ? "section-hidden" : ""}>
